...
Game.Screen.startScreen = {
    ...
    render: function(display) {
        ...
        display.drawText(1,2, "%c{yellow}Dragon Dungeon");
        display.drawText(1,3,"%c{green}Sample Text");
...
Game.Screen.playScreen = {
    ...
    enter: function() {
        ...
        var width = typelvl.width; //100
        var height = typelvl.height; //48
        var depth = 10; //6
        ...
        map.getEngine().start();
		//Render the stats in the HTML document
		...
		/*
        var div_stats;
		var div_stats_inventory_button;
		var div_stats_thirst;
		var div_stats_hp;
		var div_stats_xp;
		var div_stats_messages;
		var div_stats_level;
		div_stats = Game.HTML.createNode("p");
		div_stats_inventory_button = Game.HTML.createNode("p");
		div_stats_thirst = Game.HTML.createNode("p");
		div_stats_hp = Game.HTML.createNode("p");
		div_stats_xp = Game.HTML.createNode("p");
		div_stats_messages = Game.HTML.createNode("p");
		div_stats_level = Game.HTML.createNode("p");
		 
		 
         Game.HTML.setNodeText(div_stats,"hunger: " + hungerState);
         Game.HTML.appendNode(div_stats);
         Game.HTML.appendNodetoId(div_stats, "div_stats");
         
	   
		 
         Game.HTML.setNodeText(div_stats_thirst,"Thirst: " + thirstState);
         Game.HTML.appendNode(div_stats_thirst);
         Game.HTML.appendNodetoId(div_stats_thirst, "div_stats_thirst");
    ...
    render: function(display) {
		...
		/*============================================================================================*/
		//MessageConfig
		Game.messageConfig = new Game.createConfig("msg",function(){
			var x = [];
			x.push(messages);
			return x;
		});
		Game.messageConfig.totalMessages = Game.messageConfig.value.length;
		Game.messageConfig.currentMessage = Game.messageConfig.value[Game.messageConfig.totalMessages - 1]; //Overrides the endless list of all messages so it doesnt fill the computer's memory
		
		/*============================================================================================*/
		//ScreenConfig
		Game.ScreenConfig = new Game.Config.Config();
		Game.ScreenConfig.tag = "Screen";
		Game.ScreenConfig.value = {width: screenWidth, height: screenHeight};
		/*============================================================================================*/
		//GameConfig	(Used for player and game related variables)
		var hungerState = this._player.getHungerState();
		var thirstState = this._player.getThirstState();
		var playerHP = this._player.getHp();
		var playerMaxHP = this._player.getMaxHp();
		var playerLevel = this._player.getLevel();
		var playerEXP = this._player.getExperience();
		Game.GameConfig = new Game.Config.Config();
		Game.GameConfig.tag = "Game";
		Game.GameConfig.value = {stats: stats,
								 hunger: hungerState,
								 thirst: thirstState,
								 hp: playerHP,
								 maxhp: playerMaxHP,
        ...
        for (var i = 0; i < messages.length; i++) {
            ...
            messageY += display.drawText(
                0, 
                messageY,
                '%c{white}%b{green}' + messages[i]
            );
        }
        // Render player stats
        var stats = '%c{white}%b{red}';
        ...
        // Render hunger and thirst state
		var hungerState = this._player.getHungerState();
		var thirstState = this._player.getThirstState();
		var playerHP = this._player.getHp();
		var playerMaxHP = this._player.getMaxHp();
		var playerLevel = this._player.getLevel();
		var playerEXP = this._player.getExperience();
		//var levelDepth = Game.Player.getLevel();
        display.drawText(screenWidth - hungerState.length, screenHeight, hungerState);
		display.drawText(screenWidth - thirstState.length, screenHeight - 1, thirstState);
    ...
    handleInput: function(inputType, inputData) {
        ...
        if (inputType === 'keydown') {
            ...
            if (inputData.keyCode === ROT.VK_LEFT) {
                ...
                return;
			} else if (inputData.keyCode === ROT.VK_TAB){
...
Game.Screen.winScreen = {
    ...
    render: function(display) {
		...
		display.drawText(2, 10, "%c{green}Press [Enter] to Reload!");
    },
    handleInput: function(inputType, inputData) {
        if (inputType === 'keydown') {
            if (inputData.keyCode === ROT.VK_RETURN) {
...
Game.Screen.informationScreen = {
    enter: function() { console.log("Entered information screen."); },
    exit: function() { console.log("Exited information screen."); },
    render: function(display) {
		var text = '%c{green}Dragon Dungeon';
        var border = '-------------------------';
        var y = 0;
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, border);
       display.drawText(2,1,"Version: "+ Game.version);
	   display.drawText(2,2,"Build Number: "+ Game.build);
	   display.drawText(2,3,"Platform: "+ Game.platform);
	   display.drawText(2,4,"Made by: "+ Game.author);
	   display.drawText(2,6,"Press [ENTER] to return to the Main Menu!");
    },
    handleInput: function(inputType, inputData) {
        if (inputType === 'keydown') {
            if (inputData.keyCode === ROT.VK_RETURN) {
...
Game.Screen.loseScreen = {
    ...
    render: function(display) {
        // Render our prompt to the screen
            ...
            display.drawText(2, 1, "%b{red}You Died!");
			display.drawText(2, 5, "%c{green}Press [Enter] to Reload!");
...
Game.Screen.helpScreen = {
    render: function(display) {
        var text = 'Dragon Dungeon';
        var border = '-------------';
        var y = 0;
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, border);
        display.drawText(0, y++, 'The villagers have a old legend about a the last Dragon that lives in the cave.');
        display.drawText(0, y++, 'You set off on the cave Entrance and hope to slay the last dragon and live in riches!');
        y += 3;
        display.drawText(0, y++, '[,] to pick up items');
        display.drawText(0, y++, '[d] to drop items');
        display.drawText(0, y++, '[e] to eat items');
		display.drawText(0,y++,'[m] to drink items');
        display.drawText(0, y++, '[w] to wield items');
        display.drawText(0, y++, '[W] to wield items');
        display.drawText(0, y++, '[x] to examine items');
        display.drawText(0, y++, '[;] to look around you');
        display.drawText(0, y++, '[?] to show this help screen (in Game)');
		display.drawText(0,y++, '[>] to decend down stairs!');
		display.drawText(0,y++,'[<] to accend up stairs!');
        y += 3;
...
Game.Screen.devScreen = {
    render: function(display) {
        var text = '::Developer Mode::';
        var border = '-------------';
        var y = 0;
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, border);
        display.drawText(0, y++, 'Developer Options:');
        display.drawText(0, y++, 'Version Alpha 0.1.0');
        y += 3;
        display.drawText(0, y++, 'Numbers: 1234567890');
        display.drawText(0, y++, 'Text: abcdefghijklmnopqrstuvwxyz');
        y += 3;
        text = '--- press any key to continue ---';
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
    },
...
Game.Screen.devScreenB = {
    render: function(display) {
        var text = '::DevMode DOS Menu Subscreen information::';
        var border = '-------------';
        var y = 0;
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, border);
        display.drawText(0, y++, 'Commands:');
        display.drawText(0, y++, 'Version Alpha 0.1.0');
        y += 3;
        display.drawText(0, y++, '[F1] Game information');
        display.drawText(0, y++, '[2] Dev Option 2');
        y += 3;
        text = '--- press any key to continue, or any command ---';
        display.drawText(Game.getScreenWidth() / 2 - text.length / 2, y++, text);
    },
    handleInput: function(inputType, inputData) {
	if (inputType == 'keydown') {
        if (inputData.keyCode === ROT.VK_F1) {
