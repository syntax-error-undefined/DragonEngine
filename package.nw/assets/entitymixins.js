...
Game.EntityMixins.DragonActor = Game.extend(Game.EntityMixins.TaskActor, {
    init: function(template) {
        // Call the task actor init with the right tasks.
        Game.EntityMixins.TaskActor.init.call(this, Game.extend(template, {
            'tasks' : ['fire', 'spawnFire', 'hunt', 'wander']
        }));
        // We only want to grow the arm once.
        this._hasRageArm = false;
    },
    canDoTask: function(task) {
        // If we haven't already grown arm and HP <= 20, then we can grow.
        if (task === 'fire') {
            return this.getHp() <= 20 && !this._hasRageArm;
        // Spawn a Living Fire only a 10% of turns.
        } else if (task === 'spawnFire') {
    ...
    spawnFire: function() {
        ...
        // Create the entity
        var Fire = Game.EntityRepository.create('Fire');
        Fire.setX(this.getX() + xOffset);
        Fire.setY(this.getY() + yOffset)
        Fire.setZ(this.getZ());
        this.getMap().addEntity(Fire);
    },
    listeners: {
...
Game.EntityMixins.drinkConsumer = {
	name: 'drinkConsumer',
	init: function(template) {
		this._maxthirstness = template['maxthirstness'] || 1000;
          this._thirstness
	}
}
*/
...
Game.EntityMixins.DrinkConsumer = {
    name: 'DrinkConsumer',
    init: function(template) {
        this._maxThirst = template['maxThirst'] || 100;
        // Start halfway to max fullness if no default value
        this._thirst = /*template['thirst'] || */(this._maxthirst / 2);
        // Number of points to decrease fullness by every turn.
        this._thirstDepletionRate = /*template['thirstDepletionRate'] ||*/ 10;
    },
